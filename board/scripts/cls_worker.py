#!/usr/bin/env python3
"""
Persistent DEEPX DX-M1 image classification worker — YOLO26m-cls via dx_engine.

Protocol (binary):
  main → worker : 1 byte  (0x01 = run, 0x00 = quit)
  worker → main : 1 byte 0x01 (done; pane written to CLS_PANE_FILE)
                  float  inf_ms
"""
...
MODEL        = '/data/local/tmp/yolo26m-cls.dxnn'
FRAME_FILE   = '/tmp/dd_frame1.bin'
...
# Standard ImageNet-1k class names (ILSVRC2012)
IMAGENET_LABELS = (
    ...
    'filing cabinet','fireboat','police van','fire truck','ambulance',
    'flagpole','flute','folding chair','football helmet','forklift',
    'fountain','fountain pen','four-poster bed','freight car','French horn',
    ...
    'ladle','lampshade','laptop computer','lawn mower','lens cap','letter opener',
    'library','lifeboat','lighter','limousine','ocean liner','lipstick',
    'slip-on shoe','lotion','music speaker','magnetic compass','mailbag',
    'mailbox','tights','one-piece bathing suit','manhole cover','maraca',
    'marimba','mask','matchstick','maypole','maze','measuring cup',
    'medicine cabinet','megalith','microphone','microwave oven','military uniform',
    'milk can','minibus','miniskirt','minivan','missile','mitten','mixing bowl',
    'mobile home','Model T','modem','monastery','monitor','moped','mortar',
    'square academic cap','mosque','mosquito net','vespa','mountain bike',
    'tent','computer mouse','mousetrap','moving van','muzzle','metal nail',
    'neck brace','necklace','baby pacifier','notebook computer','obelisk',
    'oboe','ocarina','odometer','oil filter','pipe organ','oscilloscope',
    'overskirt','bullock cart','oxygen mask','product packet','paddle','paddle wheel',
    'padlock','paintbrush','pajamas','palace','pan flute','paper towel',
    'parachute','parallel bars','park bench','parking meter','railroad car',
    ...
    'pinwheel','pirate ship','pitcher','hand plane','planetarium','plastic bag',
    'plate rack','farm plow','plunger','Polaroid camera','pole','police uniform',
    'poncho','pool table','soda bottle','plant pot','potter\'s wheel',
    'power drill','prayer rug','printer','prison','missile','projectile',
    'projector','hockey puck','punching bag','purse','quill','racing car',
    'racket','radio','radio telescope','rain barrel','recreational vehicle',
    ...
    'school bus','schooner','scoreboard','sewing machine','shield','shoe store',
    ...
    'vase','viaduct','violin','volleyball','waffle iron','wall clock',
    ...
    'comic book','crossword','traffic sign','traffic light','dust jacket',
    'menu','plate','guacamole','consomme','hot pot','trifle','ice cream',
    'popsicle','baguette','bagel','pretzel','cheeseburger','hot dog',
    'mashed potato','cabbage','broccoli','cauliflower','zucchini',
    'spaghetti squash','acorn squash','butternut squash','cucumber',
    'artichoke','bell pepper','mushroom','Granny Smith apple','strawberry',
    'orange','lemon','fig','pineapple','banana','jackfruit','custard apple',
    'pomegranate','hay','carbonara','chocolate syrup','dough','meatloaf',
    'pizza','pot pie','burrito','red wine','espresso','tea cup','eggnog',
    'alp','bubble','cliff','coral reef','geyser','lakeshore','promontory',
    ...
    'bolete','ear of corn','toilet paper',
)
...
try:
    ...
    sys.stderr.write(f"[cls_worker] DEEPX init failed: {e}\n")
    sys.stderr.flush()

_pipe_out.write(b'READY\n')

# ── pre-allocate ───────────────────────────────────────────────────────────────
_inp_buf  = np.empty((INF_SIZE, INF_SIZE, 3), np.uint8)
_pane_buf = np.empty((PANE_H, PANE_W, 3), np.uint8)

def center_crop_resize(img, size=224):
    h, w = img.shape[:2]
    # Crop shorter side to square
    s = min(h, w)
    y0 = (h - s) // 2
    x0 = (w - s) // 2
    cropped = img[y0:y0+s, x0:x0+s]
    cv2.resize(cropped, (size, size), dst=_inp_buf, interpolation=cv2.INTER_AREA)
    return _inp_buf
...
while True:
    ...
    try:
        inp = center_crop_resize(raw, INF_SIZE)

        t0 = time.time()
        outs = engine.run(np.expand_dims(inp, 0))
        inf_ms = (time.time() - t0) * 1000.0

        logits = outs[0].reshape(-1).astype(np.float32)
        probs  = softmax(logits)
        top5   = np.argsort(probs)[::-1][:TOP_K]

        # Render pane: cam1 frame resized + top-5 overlay
        cv2.resize(raw, (PANE_W, PANE_H), dst=_pane_buf, interpolation=cv2.INTER_AREA)

        cv2.putText(_pane_buf, 'YOLO26m-cls  (DEEPX NPE-1)', (16, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(_pane_buf, f'cam1  {inf_ms:.1f} ms', (16, 76),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.65, (200, 200, 200), 1, cv2.LINE_AA)

        # Top-5 class labels bar (bottom 300px)
        bar_y = PANE_H - 300
        ...
        for rank, idx in enumerate(top5):
            ...
            bar_w = int(score * (PANE_W - 32))
            y = bar_y + 60 + rank * 48
            cv2.rectangle(_pane_buf, (16, y), (16 + bar_w, y + 32), (30, 100, 200), -1)
        ...
        sys.stderr.write(f"[cls_worker] inference error: {e}\n")
        sys.stderr.flush()
        try:
            np.zeros((PANE_H, PANE_W, 3), np.uint8).tofile(CLS_PANE_FILE)
        except Exception:
