#!/usr/bin/env python3
"""Write edge_art_4pane.sh with Unix LF line endings, copied from original imdt-deepx-demo.sh topology."""
import os
...
lines = [
    ...
    r'YOLO_V8_POST="qtimlvdetection threshold=50 results=10 labels=${YOLO_V8_LABELS} module=yolov8 constants=\"${YOLO_V8_CONSTANTS}\""',
...
out = os.path.join(os.path.dirname(__file__), "scripts", "edge_art_4pane.sh")
with open(out, "w", newline="\n") as f:
    f.write("\n".join(lines))
print(f"Written {out} ({os.path.getsize(out)} bytes, LF-only)")
